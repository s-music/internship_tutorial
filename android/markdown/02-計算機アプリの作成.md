...
fun onClickSampleButton(view: View) {
    ...
    var text = number1EditText.text.toString()
...
fun onClickSampleButton(view: View) {
    ...
    var num1 = number1EditText.text.toString().toIntOrNull() ?: 0
...
関数の戻り値を処理している構文はスマートキャストと呼ばれる構文で、  
resultがInt型だった場合if節を実行し、その中ではresultを自動的にInt型に変換してくれます。  
また、resultがInt型でなかった場合は一般的なif文と同様にelse節を実行します。  
